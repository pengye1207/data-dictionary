...
public class MySqlDao implements IDao {
    ...
    private static final String TABLES_SQL =
        "SELECT TABLE_NAME,TABLE_COMMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = ? ORDER BY TABLE_NAME ASC";
    ...
    private static final String TABLES_COLUMN_SQL = ""
        ...
        + "       IF(COL.COLUMN_KEY = 'PRI', 'Yes', 'No')      AS COLUMN_KEY,\n"
        + "       IF(STA.INDEX_NAME IS NULL, 'None', STA.INDEX_NAME) AS INDEX_NAME,\n"
        + "       IF(CON.CONSTRAINT_NAME IS NULL, 'No', 'Yes') AS COLUMN_UNIQUE,\n"
        + "       IF(COL.IS_NULLABLE = 'YES', 'Yes', 'No')     AS IS_NULLABLE,\n"
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(TABLES_SQL)) {
            ...
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                ...
                while (resultSet.next()) {
                    ...
                    tables.add(new Table().setName(tableName).setComment(resultSet.getString("TABLE_COMMENT")));
                }
                return tables;
            }
        } catch (Exception e) {
            ...
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    tableColumns.computeIfAbsent(tableName, k -> new ArrayList<>())
                        .add(new TableColumn().setTableName(tableName)
